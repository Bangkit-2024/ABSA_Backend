...
from content.models import Review, ReviewAspectSentiment, Company
...
def handle_bulk_aspect_based_review(df:pd.DataFrame):
    ...
    with transaction.atomic():
        for _,row in df.iterrows():
            ...
            review_absa = ReviewAspectSentiment(
                review = review,
                aspect = row['aspect'],
...
def handle_bulk_review_only(df:pd.DataFrame):
